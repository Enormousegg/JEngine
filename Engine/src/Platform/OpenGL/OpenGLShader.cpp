...
namespace Engine {
	...
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		EG_CORE_ASSERT(false, "Unknow shader type!");
		return 0;
	}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			EG_CORE_ASSERT("Could not open file '{0}'", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			size_t begin = pos + typeTokenLength + 1;//只考虑一个空格的情况 
			std::string type = source.substr(begin, eol - begin);
			EG_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specific");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSource)
	{
		...
		EG_CORE_ASSERT(shaderSource.size() <= 2, "Only support 2 shaders for now");
		...
		if (isLinked == GL_FALSE)
		{
			...
			EG_CORE_ERROR("{0}", infoLog.data());
			EG_CORE_ASSERT(false, "Shader linked failure!");
			return;
		}
